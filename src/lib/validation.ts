...
export function validatePlayerInput(body: unknown): ValidationResult {
  ...
  if (data.number !== undefined && (typeof data.number !== 'number' || data.number < 0 || data.number > 99)) {
    errors.push('Дугаар 0-99 хооронд байх ёстой');
  }
  
  if (data.age !== undefined && (typeof data.age !== 'number' || data.age < 0 || data.age > 100)) {
...
export function validateTeamInput(body: unknown): ValidationResult {
  ...
  if (!data.shortName || typeof data.shortName !== 'string' || data.shortName.trim().length < 2) {
