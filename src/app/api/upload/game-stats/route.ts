  ...
  try {
    ...
    if (ext !== "xlsx" && ext !== "xls") {
      return errorResponse(
        "Зөвхөн Excel файл (.xlsx, .xls) зөвшөөрөгдөнө",
        400,
      );
    }
    ...
    if (!isValidExcel) {
      return errorResponse(
        "Буруу файл формат - зөвхөн Excel файл зөвшөөрөгдөнө",
        400,
      );
    }
    ...
    if (!sheet) {
      return errorResponse("Excel файл хоосон байна", 400);
    }
    ...
    data.forEach((row: any) => {
      ...
      if (player) {
        ...
        player.stats.fieldGoalsMade += parseInt(
          row.FGM || row["FG Made"] || "0",
        );
        player.stats.fieldGoalsAttempted += parseInt(
          row.FGA || row["FG Attempted"] || "0",
        );
        player.stats.threePointersMade += parseInt(
          row["3PM"] || row["3PT Made"] || "0",
        );
        player.stats.threePointersAttempted += parseInt(
          row["3PA"] || row["3PT Attempted"] || "0",
        );
        player.stats.freeThrowsMade += parseInt(
          row.FTM || row["FT Made"] || "0",
        );
        player.stats.freeThrowsAttempted += parseInt(
          row.FTA || row["FT Attempted"] || "0",
        );
