...
// Columns for the manual input table (matches CSV_HEADERS from constants.ts)
const MANUAL_COLUMNS = [
  ...
  { key: "totalRebounds", label: "REB", type: "number", width: 55 },
  { key: "assists", label: "AST", type: "number", width: 50 },
  { key: "turnovers", label: "TO", type: "number", width: 50 },
  { key: "steals", label: "STL", type: "number", width: 50 },
...
/** Convert manual rows into a CSV string (same format the Cloud Function expects). */
...
export default function AdminUploadCsv() {
  ...
  // -----------------------------------------------------------------------
  // Submit handler (works for both CSV file and manual input)
  // -----------------------------------------------------------------------
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setResult(null);

    if (!seasonId || !selectedGameId || !selectedTeamId) {
      setResult({
        ok: false,
        error: "Улирал, тоглолт, баг сонгоно уу.",
      });
      return;
    }
    ...
    if (mode === "csv") {
      ...
      // Manual mode → convert rows to CSV blob
        ...
        {mode === "manual" && (
          <div style={{ margin: "15px 0", overflowX: "auto" }}>
            <table
              ...
              style={{ fontSize: 13, minWidth: 1200 }}
            >
              <thead>
                <tr>
                    ...
                    {MANUAL_COLUMNS.map((col) => (
                      <td key={col.key} style={{ padding: 2 }}>
                        <input
                          ...
                          placeholder={
                            "placeholder" in col
                              ? (col as { placeholder: string }).placeholder
                              : ""
                          }
                          style={{
                            width: "100%",
