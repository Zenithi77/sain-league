...
export default function ScheduleClient({ games }: ScheduleClientProps) {
  ...
  return (
    <div className="schedule-wrapper">
      ...
      <div className={`selected-date-header ${isToday(selectedDate) ? 'is-today' : ''}`}>
        <div className="date-info">
          <h2>
            {isToday(selectedDate) ? (
              ...
              <>
                <i className={`fas ${isPast(selectedDate) ? 'fa-history' : 'fa-calendar-day'}`}></i>
                <span className="date-text">{formatDisplayDate(selectedDate)}</span>
              </>
            )}
          </h2>
      ...
      {filteredGames.length === 0 && (
        <div className="no-games-message">
          ...
          <p>Энэ өдөр тоглолт {isPast(selectedDate) ? 'болоогүй' : 'товлогдоогүй'} байна</p>
      ...
      {finishedGames.length > 0 && (
        <section className="schedule-section finished-section">
          <div className="section-header">
            ...
            <span className="section-count">{finishedGames.length} тоглолт</span>
          </div>
          <div className="games-grid">
      ...
      {scheduledGames.filter(g => g.status !== 'live').length > 0 && (
        <section className="schedule-section upcoming-section">
          <div className="section-header">
            <h3><i className="fas fa-clock"></i> Удахгүй болох тоглолтууд</h3>
            <span className="section-count">{scheduledGames.filter(g => g.status !== 'live').length} тоглолт</span>
          </div>
          <div className="games-grid">
            {scheduledGames.filter(g => g.status !== 'live').map((game) => (
              <GameCard key={game.id} game={game} />
            ))}
          </div>
        </section>
